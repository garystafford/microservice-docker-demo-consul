...
"docker rm -f consul-server-1 || echo 'No container exists?'"
...
ssh -oStrictHostKeyChecking=no -i ~/.ssh/consul_aws_rsa ubuntu@${ec2_public_ip} \
"docker rm -f consul-server-2 || echo 'No container exists?'"
...
ssh -oStrictHostKeyChecking=no -i ~/.ssh/consul_aws_rsa ubuntu@${ec2_public_ip} \
  "docker rm -f consul-server-3 || echo 'No container exists?'"
